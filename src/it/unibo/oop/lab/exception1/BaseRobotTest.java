...
public final class BaseRobotTest {
    ...
    public void testRobotMovementBase() {
        ...
        for (int i = 0; i < RobotEnvironment.WORLD_X_UPPER_LIMIT; i++) {
        	...
        	try {
    			...
    			assertNotNull("Exception throwed without message", e.getMessage());
    			fail("Position out of bounds while checking moving up");
    		}
        }
        ...
        try {
			...
			assertNotNull("Exception throwed without message", e.getMessage());
			fail("Position out of bounds while checking moving right");
		}
    ...
    public void testRobotBatteryBase() {
        ...
        try {
			...
			assertNotNull("Exception throwed without message", e.getMessage());
			fail("Not Enough battery while checking moving up");
        }
        // recharge battery
        r2.recharge();
        // verify battery level
